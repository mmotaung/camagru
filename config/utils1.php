...
if (isset($_POST['submit']) && isset($_POST['passwd']))
{
    ...
    if ($passwd == "")
    {
        header("Location: ../utils.php?pass");
        exit();
    }
    else
    {
        ...
        try
        {
            ...
            if (count($result))
            {
                foreach ($result as $row)
                {
                    ...
                    {
                        header("Location: ../utils.php?pass_incorrect");
                        exit();
                    }
                }
            }
            ...
            {
                header("Location: ../utils.php?unable_to_connect");
                exit();
            }
        }
        catch(PDOException $e)
        {
            header("Location: ../utils.php?server_error");
            exit();
        }
        ...
        if ($username == "" && $email == "")
        {
            header("Location: ../utils.php?empty");
            exit();
        }
        else
        {
            try
            {
                ...
                $conn4 = new PDO($db_dsn, $db_username, $db_password);
                ...
                foreach ($results as $row4)
                {
                    ...
                    {
                        header("Location: ../utils.php?username");
                        exit();
                    }
                    else if ($row['email'] == $email)
                    {
                        header("Location: ../utils.php?email");
                        exit();
                    }
                }
            }
            ...
            try
            {
                ...
                if (count($result))
                {
                    ...
                    try
                    {
                        ...
                        //success url message
                        header("Location: ../utils.php?success");
                        exit();
                    }
                    catch(PDOException $e)
                    {
                ...
                {
                    header("Location: ../utils.php?user_not_found");
                    exit();
                }
